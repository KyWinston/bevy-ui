...
use crate::{styles::*, components::{Screen, RespawnButton, BasicButtonBundle, ButtonTextBundle, QuitButton}};

use super::components::Victory;

pub fn spawn_victory_screen(mut commands: Commands, asset_server: Res<AssetServer>) {
    build_victory_screen(&mut commands, &asset_server);
}

pub fn build_victory_screen(commands: &mut Commands, asset_server: &Res<AssetServer>) -> Entity {
    let victory_entity = commands
        .spawn((
            NodeBundle {
